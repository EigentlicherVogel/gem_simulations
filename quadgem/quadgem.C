#define UNUSED(expr) do { (void)(expr); } while (0)
    ...
    int main(int argc, char * argv[]) {
      ...
      */
      ...
      TString filename;
      filename = Form("OUTFILE/quadgem_gemout_91ArCo2_290dv_051pen2x_%s.root", argv[1]);
      ...
      elistf = Form("ANSYS_geo/ELIST_%s.lis", argv[2]);
      nlistf = Form("ANSYS_geo/NLIST_%s.lis", argv[2]);
      mplistf = Form("ANSYS_geo/MPLIST_%s.lis", argv[2]);
      prnsolf = Form("ANSYS_geo/PRNSOL_%s.lis", argv[2]);
	...
	fieldView4->SetPlane(0., -1., 0., 0., 0., 0.);
      ...
      gas->EnablePenningTransfer(0.51, 0.0, "ar");
      gas->Initialise(true);
      // gas->Initialise();
