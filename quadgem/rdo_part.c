...
int main(int argc, char *argv[]) {
  ...
  TString filename;
  filename = Form("OUTFILE/chev_single_%s.root", argv[1]);
  ...
  rdo_board->Initialise("ANSYS_geo/ELIST_ro_chevfix2.lis",
                  "ANSYS_geo/NLIST_ro_chevfix2.lis",
                  "ANSYS_geo/MPLIST_ro_chevfix2.lis",
                  "ANSYS_geo/PRNSOL_ro_chevfix2.lis", "mm");
  rdo_board->EnablePeriodicityX();
 ...
 // rdo_board->SetMagneticField(0,by,bz);
  ...
  TCanvas *cpr1 = new TCanvas("cpr1", "", 600, 600);
  TCanvas *cfr1 = new TCanvas("cfr1", "", 600, 600);
  TCanvas *cpr2 = new TCanvas("cpr2", "", 600, 600);
  TCanvas *cfz = new TCanvas("cfz", "", 600, 600);
  const double vpitch = 3.252; // in cm
  const double hpitch = 0.300; // in cm
  ...
  if (plotField) {
    ...
    cpr1->SetLeftMargin(0.16);
    fieldView->SetCanvas(cpr1);
    fieldView->PlotProfile(0., 0., 0.1, 0., 0., 0);

    // Set the viewing plane (normal vector) and ranges
    fieldView->SetPlane(0., -1., 0., 0., 0., 0.005);
    // fieldView->SetArea(-pitch / 2., -0.02, pitch / 2., 0.02);
    fieldView->SetArea(-hpitch / 2, -0.15, hpitch, 0.15);

    fieldView->SetVoltageRange(-250., 0.); 
    // TCanvas* cf = new TCanvas("cf", "", 600, 600);
    cfr1->SetLeftMargin(0.16);
    fieldView->SetCanvas(cfr1);
    fieldView->PlotContour();

    ViewField *fieldView2 = new ViewField();
    fieldView2->SetComponent(rdo_board);
    // Plot the potential along the hole axis.
    cpr2->SetLeftMargin(0.16);
    fieldView2->SetCanvas(cpr2);
    fieldView2->PlotProfile(0., 0.2, 0., 0., -0.2, 0.);
    
    fieldView2->SetPlane(0., 0., 0.1, 0., 0., 0.0053);
    fieldView2->SetArea(-0.9, -0.3, 0.9, 0.3);
    fieldView2->SetVoltageRange(-250., 0.); 
    // TCanvas* cf = new TCanvas("cf", "", 600, 600);
    cfz->SetLeftMargin(0.16);
    fieldView2->SetCanvas(cfz);
  ...
  */
  ...
  if (plotDrift) {
    driftView->SetArea(-1.0 * hpitch, -1.0 * vpitch, 0, 1 * hpitch, 1 * vpitch,
                       0.05);
  ...
  for (int i = 0; i < nevents; i++) {
    ...
    etree->GetEntry(i);
    revent = i;
    Float_t electron_zcutoff = -0.4;

    for (int j = 0; j < ex2->size(); j++) {

      if ((ez2->at(j) > electron_zcutoff))
        continue;
      xi = ex2->at(j);
      yi = ey2->at(j);
