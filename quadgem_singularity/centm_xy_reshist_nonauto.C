void centm_xy_reshist()
{
  ...
  TH1* finalhist_x = new TH1S("txrx", "truth x - recon x", 80, -0.006, 0.006);
  TH1* finalhist_y = new TH1S("tyry", "truth y - recon y", 80, -0.006, 0.006);
  ...
  tch_fln->AddFile("xyrdo_xy45ite_quadgem350_6mmtotal_30um_1.root");
  int max_iter = tch_fln->GetEntries();
    ...
    cout << "X init will be from " << offsetXmin << " " << offsetXmax << endl; 
    cout << "Y init will be from " << offsetYmin << " " << offsetYmax << endl; 
    delete htempx;
    delete htempy;
  ...
  TFile *store_file = new TFile("xy_iterativexy_offsrd_aggr.root","RECREATE");
  ...
  for(int fl = 1;fl <= file_ct;fl++){
    ...
    sprintf(filename_iter, "xyrdo_xy45ite_quadgem350_6mmtotal_30um_%d.root", fl);
