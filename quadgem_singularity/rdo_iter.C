...
//   filename = Form("./OUTFILE/chevrdo_rdoxite_quadgem350_expcld1_5_40um300c_%s.root", argv[1]);
   filename = Form("./OUTFILE/xyrdo_73ArCo2_350dv_50pi-xyot_xy20i3u_xbfield15_%s.root", argv[1]);
  ...
  */
  ...
  // Loading the Ansys files (weighting afterwards)
      ComponentAnsys123* gem = new ComponentAnsys123();
        ...
        TString lsrs = "rdo_xy_90deg_fix4"; 
...
sensor->SetArea(-32 * pitch, -48 * pitch, 0,
          32 * pitch,  32 * pitch,  0.05);
  ...
  fileinp = Form("./OUTFILE/quadgem_gemout_73ArCo2_350dv_50vertpi-_24f15bf_%s.root", argv[1]);
  //
  ...
  for(int l = 1; l <= it; l++){
    ...
    for(int i=0; i < gnevents; i++) {
      ...
      Double_t iteration_movement = 0.00212; //30u 45d
      ...
      cout << " e- drift init z :" << e_drift << endl;
          ...
          for(int j = 0 ; j < ex2->size(); j++) {
            ...
            // 	yi = (ey2->at(j)*multip_cloudwidth)+const_y_offset;
            	...
            	zi = (ez2->at(j)) - (ez2->at(j)) + 0.0460; // This is 0.1 cm drift (+0.1 cm to move the previous stage electron to R0 @z=0 and +0.1 cm to move them further up to drift region of RO geometry
            	ei = ee2->at(j);
            	ti = et2->at(j);
