...
//   filename = Form("./OUTFILE/chevrdo_rdoxite_quadgem350_expcld1_5_40um300c_%s.root", argv[1]);
   filename = Form("OUTFILE/quadgem_gemout_73ArCo2_350dv_itlay_2_%s.root", argv[1]);
  ...
  */
  ...
  if (plotField) {
    ...
    fieldView2->PlotProfile(0., 0., 0.1, 0., 0., -0.1);
  ...
  */
  ...
  fileinp = Form("OUTFILE/quadgem_gemout_73ArCo2_350dv_itlay_1_%s.root", argv[1]);
  //
      ...
      cout << " e- drift init z :" << e_drift << endl;
          ...
          for(int j = 0 ; j < ex2->size(); j++) {
            ...
            // 	yi = (ey2->at(j)*multip_cloudwidth)+const_y_offset;
            	...
            	zi = (ez2->at(j)) - (ez2->at(j)) + 0.095; // This is 0.1 cm drift (+0.1 cm to move the previous stage electron to R0 @z=0 and +0.1 cm to move them further up to drift region of RO geometry
            	ei = ee2->at(j);
            	ti = et2->at(j);
