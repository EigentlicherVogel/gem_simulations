#define UNUSED(expr) do { (void)(expr); } while (0)
    ...
    int main(int argc, char * argv[]) {
      ...
      Float_t mesh =   0.295;    //Coordinate of drift electrode/cathode along Z  in cm
      ...
      */
      int nevents=20; // Number of events.
      /*
      if (argv[6]) {
        nevents = atoi(argv[6]);
      ...
      */
      ...
      TString filename;
      filename = Form("OUTFILE/quadgem_gemout_73ArCo2_350dv_itlay_1_%s.root", argv[1]);
...
TCanvas *cp1 = new TCanvas("cp1", "", 600, 600);
  ...
  if (plotField) {
    ...
    fieldView->SetArea(-pitch / 2, -pitch / 2, pitch / 2, pitch / 2);

    fieldView->SetVoltageRange(-3500., 0.); // gem example
    ...
    fieldView2->SetArea(-pitch / 2, -0.11, pitch / 2, 0.1);
    fieldView2->SetVoltageRange(-3500., 500.);
    // TCanvas* cf = new TCanvas("cf", "", 600, 600);
    ...
    fieldView2->PlotContour();
      ...
      sensor->SetArea(-12* pitch, -12* pitch,-0.1,
              12* pitch,  12* pitch,   0.3  );
